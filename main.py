...
🔖 VERSİYON: v16.60 (SADECE MANUEL + TEYITLI + 4 TP + TP1 TABAN + 1H-VOLATILITE SL + ACIK-POZ DUZELTME + KURTARMA-TP + 4 sabit TP - VUR KAÇ %30/25/25/20 tam kapanış + hizli ac/kapat + teyit bekleme + kademeli SL yukseltme + 4-bilesenli trend teyidi (1h mum yonu dahil) + scalp oz tarama[VARSAYILAN KAPALI] + coklu kanal + manuel komutlar tekrar anında acilir (teyitsiz) + ANI HAREKET tespiti (Gir/Pas butonu))
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
# v16.60: AÇILIŞ-KURTARMA YARIŞ DURUMU DÜZELTMESİ.
...
# atlar.
islem_aciliyor = {}  # {symbol: baslama_zamani} — v16.60
...
PIN_MIN_ARALIK_SN = 10  # v16.60: art arda gelen SL/TP/hard-stop güncellemeleri
...
ANI_HAREKET_WATCHLIST_BOYUTU = int(os.getenv("ANI_HAREKET_WATCHLIST_BOYUTU", "30"))  # v16.60: 60->30, tur süresi kısalsın
...
ANI_HAREKET_TARAMA_ARALIK_SN = int(os.getenv("ANI_HAREKET_TARAMA_ARALIK_SN", "20"))  # v16.60: 45->20, daha hızlı yakalasın
...
def ani_hareket_1_2saat_tespit_et(sym):
    """
    v16.60 — kullanıcı geri bildirimi: "son 1 saatte 2 saatte yükselen/düşeni
    ...
    try:
        ...
        # v16.60: eşik %0.3'ten %0.6'ya çıkarıldı — kullanıcı gözlemi: %0.3
        ...
        son_tepe = max(highs[-9:])
        if yon == "long":
            geri_cekilme_pct = (son_tepe - fiyat_simdi) / son_tepe * 100 if son_tepe else 0
            if geri_cekilme_pct < 0.6:
                return None
...
def ani_hareket_tespit_et(sym):
    ...
    try:
        ...
        # yükseğinden EN AZ %0.6 geri çekilmiş olması şart.
        highs3 = [c[2] for c in m3]
        son_tepe = max(highs3[-6:])
        if yon_tahmini == "long":
            geri_cekilme_pct = (son_tepe - fiyat_simdi) / son_tepe * 100 if son_tepe else 0
            if geri_cekilme_pct < 0.6:
                return None  # hâlâ tam tepede/tepeye çok yakın — bekle, bir sonraki turda tekrar bakılacak
...
def ani_hareket_tarama_loop():
    ...
    # v16.60: RWA (tokenize edilmiş hisse/ETF, örn. MUUUSDT = Direxion MU Bull
    ...
    # kontrat bilgisinde isRwa='YES' ile işaretleniyor — bu coinler kripto gibi
    # 7/24 işlem görmediği için ani hareket taramasından TAMAMEN hariç
    # tutuluyor. Liste periyodik olarak (her turda değil, ilk turda ve sonra
    # nadiren) tazeleniyor, RWA listesi sık değişmez.
    rwa_semboller = set()
    try:
        piyasalar = exchange.fetch_markets()
        for m in piyasalar:
            bilgi = m.get("info", {}) or {}
            if str(bilgi.get("isRwa", "")).upper() == "YES":
                rwa_semboller.add(m.get("symbol"))
        log.info(f"[ANI_HAREKET] {len(rwa_semboller)} RWA (tokenize hisse) sembolü hariç tutulacak")
    except Exception as e:
        log.warning(f"[ANI_HAREKET] RWA listesi alınamadı, filtre uygulanamayacak: {e}")
        ...
        try:
            ...
            for sym, t in tickers.items():
                ...
                if sym in rwa_semboller:
                    continue  # v16.60: tokenize hisse/ETF — kripto gibi 7/24 işlem görmüyor, atla
                vol = safe(t.get("quoteVolume"))
                if vol < 3_000_000:  # v16.60: likidite tabanı düşürüldü (2.5M-8M arası
                    continue          # gerçek yükselenler de kaçmasın diye)
                # v16.60: EN ÖNEMLİ DEĞİŞİKLİK — kullanıcı geri bildirimi: "şu an
            ...
            for sym in watchlist:
                ...
                # v16.60: Kullanıcı kararı — hız yerine GÜVENİLİRLİK tercih edildi.
                ...
                # v16.60: kullanıcı kararı DEĞİŞTİ — "otomatik işlem açmayı
                ...
                #   1) SADECE LONG (v16.60'ten kalan karar hâlâ geçerli)
...
def hizli_sinyal_ayristir(metin):
    ...
    v16.60 DÜZELTME: "1000XEC", "1MBABYDOGE", "1000PEPE" gibi RAKAMLA
    BAŞLAYAN coin isimleri (kripto'da çok yaygın bir isimlendirme —
    "1000x" katları anlamına gelir) eskiden HİÇ tanınmıyordu — regex
    sembolün İLK karakterinin harf olmasını zorunlu kılıyordu ("1000xec
    long" yazınca eşleşme oluşmuyordu, komut sessizce hiçbir şey yapmıyordu).
    Şimdi ilk karakter de rakam olabiliyor.
    """
    m = re.search(r"\b([A-Za-z0-9]{2,15})\b.*?\b(LONG|SHORT)\b", metin, re.IGNORECASE)
    if not m:
        return None
...
# POZİSYON BOYUTU
# ════════════════════════════════════════════
def efektif_kaldirac_al(sym):
    """
    v16.60: Her coin'in borsadaki MAKSİMUM izin verilen kaldıracı farklı
    olabiliyor (bazıları 20x, bazıları sadece 10x destekliyor). Eskiden
    sabit LEV (örn. 20) her coin için körlemesine deneniyordu — coin
    10x'i aşamıyorsa set_leverage() BAŞARISIZ oluyordu ("işlem atlandı")
    YA DA borsa sessizce daha düşük bir kaldıraç uyguluyordu ama miktar
    hesabı hâlâ istenen (20x) kaldıraca göre yapıldığı için "bakiye
    yetersiz" hatası çıkıyordu (RIVER/USDT örneği). Şimdi coin'in gerçek
    maksimum kaldıracı borsadan sorgulanıp, LEV ile bunun KÜÇÜK OLANI
    kullanılıyor — "borsanın kaldıracı ne ise onunla açsın".
    """
    try:
        piyasa = exchange.market(sym)
        bilgi = piyasa.get("info", {}) or {}
        maks_kaldirac = int(float(bilgi.get("maxLever", LEV)))
        return min(LEV, maks_kaldirac) if maks_kaldirac > 0 else LEV
    except Exception as e:
        log.warning(f"[KALDIRAÇ] {sym} maksimum kaldıraç alınamadı, varsayılan {LEV}x deneniyor: {e}")
        return LEV


def pozisyon_boyutu_hesapla(entry, sl, lev=None):
    """
    ...
    v16.60: `lev` parametresi eklendi — verilmezse global LEV kullanılır,
    verilirse (efektif_kaldirac_al'dan gelen, coin'e özel) o kullanılır.
    """
    if entry <= 0:
        return None, None
    kullanilacak_lev = lev if lev else LEV
    notional = MARGIN_SABIT * kullanilacak_lev
...
def gercek_bakiye_yeterli_mi(gereken_marj):
    ...
    v16.60 DÜZELTME: STRK/USDT gibi örneklerde, bu kontrol GEÇMESİNE rağmen
    borsa yine de "amount exceeds balance" (40762) hatası veriyordu. Sebep:
    Bitget, gerekli marjın üstüne komisyon + fiyat dalgalanma payı (kontrat
    bilgisindeki openCostUpRatio, genelde ~%1-2) EKLİYOR — bizim hesabımız
    tam sınırda kalınca bu ek pay yüzünden borsa reddediyordu. Şimdi
    gereken marjın üstüne %8 güvenlik payı ekleniyor (borsanın gerçek
    payından daha cömert, garanti altına almak için).
    """
    try:
    ...
    gereken_marj_payli = gereken_marj * 1.08  # v16.60: %8 güvenlik payı
    return serbest_usdt >= gereken_marj_payli
...
# ── TP DİLİM AĞIRLIKLARI (v16.60 — 4 SABİT TP + KALAN %15 TRAILING) ──
# 5 TP'den, kullanıcı kararıyla 4 sabit TP + trailing'e döndü: "4 TP vurdu,
# toplam kâr düşüktü, kalanı trailing'le devam etsin mi?" — cevap evet.
# TP1-4 sabit hedef (0.25+0.20+0.20+0.20=0.85), KALAN %15 artık TP5'i
# beklemiyor — TP4 vurulur vurulmaz otomatik TRAILING moduna geçiyor
# (trailing kodu zaten mevcuttu, sadece TP_SAYISI_KULLANILAN=4 yapılınca
# "son_tp" TP4'e denk geliyor ve tetikleniyor). Mantık: 4/4 TP vurmuş bir
# pozisyon güçlü bir trend içinde demektir, kalan küçük dilimde SL zaten
# TP4 seviyesine kilitlenmiş (aşağı risk yok) — trend'in daha da
# gitmesinden faydalanmak için sabit bir tavana takılı kalmamak mantıklı.
TP_DILIM_ORANLARI = [0.25, 0.20, 0.20, 0.20]
...
MIN_TP1_R_ORANI = 0.75  # v16.60: 0.65'ten 0.75'e yükseltildi — kullanıcı talebi:
                        # "TP'lerin aralarını biraz açalım", SL'e dokunmadan
                        # (SL genişletmenin sadece kaybı büyüttüğü, kazancı
                        # artırmadığı ayrıca konuşuldu — bu yüzden SL sabit kaldı,
                        # sadece TP tarafı daha cömert hedeflere çekildi).
...
def tp_olcekle(entry, sl, tp_liste, direction, carpan=TP_OLCEK_CARPANI):
    ...
    # ÖNCEKİNDEN en az %35 daha uzakta olacak şekilde zorlanıyor (v16.60:
    # %20'den %35'e çıkarıldı — "TP'lerin aralarını açalım" talebi) —
    # sıralama (TP1<TP2<TP3<...) HER ZAMAN korunuyor.
    for i in range(1, len(yeni_oranlar)):
        min_gerekli = yeni_oranlar[i-1] * 1.35
...
def sinyal_guc_skoru(rsi_1h, mum_4h_sayisi, mum_1h_sayisi, direction):
    """
    v16.60 — kullanıcı talebi: "sinyale yüzde koysan, çok güçlüyse bilelim".
...
def trend_teyidi_yeterli_mi(sym, direction):
    ...
    try:
        ...
        if direction == "long":
            ...
            rsi_yeterli = 40 < rsi_1h < 80  # v16.60: ÜST SINIR eklendi — RSI 92.9
            ...
            rsi_yeterli = 20 < rsi_1h < 60  # v16.60: ALT SINIR eklendi — aynı mantık
...
def sinyali_isle(sinyal):
    ...
    # v16.60: kullanıcı kararı — manuel komutlar, kanal sinyalleri ve ani
    # hareket buton onayları hepsi ANINDA (teyitsiz) açılıyor. Sadece bot'un
    # kendi otomatik ani-hareket taraması (ani_hareket_tarama_loop) hâlâ
    # trend_teyidi_yeterli_mi + kalite skoru filtresinden geçip Gir/Pas
    # bildirimi gönderiyor, kullanıcı onayı olmadan açmıyor.
...
def asil_islemi_ac(sinyal, gozlem_str=""):
    ...
    # v16.60: SEMBOL ADI UYUŞMAZLIĞI düzeltmesi — kullanıcı gözlemi: bazı
    # coinler "isimlerinden dolayı" açılmıyordu. Kanal/manuel'den gelen
    # sembol borsada TAM olarak bulunamazsa (örn. borsanın gerçek temel
    # sembolü farklıysa), TABAN COIN ADINA göre büyük/küçük harf duyarsız
    # bir eşleşme aranıyor — bulunursa DÜZELTİLMİŞ sembolle devam ediliyor,
    # bulunamazsa net bir hata mesajıyla durduruluyor (sessizce başarısız
    # olmak yerine).
    try:
        exchange.market(sym)
    except Exception:
        aranan_taban = sym.split("/")[0].upper()
        bulunan = None
        try:
            for aday_sym, aday_bilgi in exchange.markets.items():
                if not aday_sym.endswith("/USDT:USDT"):
                    continue
                if aday_sym.split("/")[0].upper() == aranan_taban:
                    bulunan = aday_sym
                    break
        except Exception:
            pass
        if bulunan:
            tg(f"ℹ️ {sym} borsada tam eşleşmedi, en yakın sembol kullanılıyor: {bulunan}")
            sym = bulunan
        else:
            tg(f"⚠️ {sym} borsada bulunamadı (ne tam ne yakın eşleşme) — bu coin bu borsada "
               f"USDT perpetual olarak işlem görmüyor olabilir, işlem açılmadı.")
            return

    # v16.60: RWA (tokenize hisse/ETF) GÜVENLİK KONTROLÜ — hangi yoldan
    ...
    # durumuna girip pozisyon KAPATILAMAZ hâle geldi. Bu ürünler kripto
    # gibi 7/24 işlem görmüyor, riskli — tamamen reddediliyor.
    try:
        piyasa_bilgi = exchange.market(sym)
        if str((piyasa_bilgi.get("info", {}) or {}).get("isRwa", "")).upper() == "YES":
            tg(f"⛔ {sym} reddedildi — bu tokenize bir hisse/ETF ürünü (RWA), "
               f"kripto gibi 7/24 işlem görmüyor, güvenlik gereği açılmıyor.")
            return
    except Exception as e:
        log.warning(f"[RWA_KONTROL] {sym} kontrol edilemedi, devam ediliyor: {e}")
    ...
    # v16.60: coin'in GERÇEK maksimum kaldıracı önce belirlenip, tüm
    # sonraki hesaplarda (miktar, marj kontrolü, set_leverage) TUTARLI
    # şekilde kullanılıyor — istenen (LEV) ile borsanın izin verdiği
    # farklıysa artık "bakiye yetersiz" gibi yanlış hatalar çıkmıyor.
    efektif_lev = efektif_kaldirac_al(sym)
    if efektif_lev < LEV:
        tg(f"ℹ️ {sym} için borsa maksimum {efektif_lev}x kaldıraca izin veriyor "
           f"(istenen {LEV}x yerine {efektif_lev}x kullanılacak)")

    amount, notional = pozisyon_boyutu_hesapla(entry_hedef, sl, lev=efektif_lev)
    if not amount:
        tg(f"⚠️ {sym} pozisyon boyutu hesaplanamadı, atlandı")
        return

    gereken_marj = notional / efektif_lev
    ...
    try:
        exchange.set_leverage(efektif_lev, sym)
    except Exception as e:
        tg(f"⚠️ {sym} kaldıraç {efektif_lev}x olarak ayarlanamadı ({e}) — "
           f"borsada o an aktif olan kaldıraçla devam ediliyor")
    ...
    with islem_aciliyor_lock:
        islem_aciliyor[sym] = time.time()  # v16.60: manage()'in kurtarma bloğu bu sembolü açılış bitene kadar atlasın

    # v16.60: "amount exceeds balance" (40762) hatasına karşı otomatik
    # küçültme ile yeniden deneme — bazı coinlerde ön kontrolümüz geçse
    # bile borsa ekstra bir maliyet payı yüzünden reddedebiliyor (STRK/USDT
    # örneği). İlk denemede olmazsa miktarı %10 azaltıp tekrar dener.
    acilis_emri = None
    denenecek_miktarlar = [qty, qty * 0.90, qty * 0.80]
    son_hata = None
    for i, deneme_qty in enumerate(denenecek_miktarlar):
        try:
            deneme_qty_hassas = float(exchange.amount_to_precision(sym, deneme_qty))
            if deneme_qty_hassas <= 0:
                continue
            acilis_emri = exchange.create_market_order(sym, side, deneme_qty_hassas)
            qty = deneme_qty_hassas  # gerçekte açılan miktarı yansıt
            if i > 0:
                tg(f"ℹ️ {sym} ilk denemede bakiye hatası alındı, miktar %{(1-deneme_qty/denenecek_miktarlar[0])*100:.0f} "
                   f"küçültülerek başarıyla açıldı")
            break
        except Exception as e:
            son_hata = e
            hata_metni = str(e)
            if "40762" not in hata_metni and "exceeds the balance" not in hata_metni.lower():
                break  # bakiye dışı bir hata — küçültmenin faydası yok, direkt çık
            continue  # bakiye hatası — bir sonraki (daha küçük) miktarı dene

    if acilis_emri is None:
        tg(f"⚠️ {sym} giriş emri başarısız (miktar küçültme denemeleri dahil): {son_hata}")
        with islem_aciliyor_lock:
    ...
    with islem_aciliyor_lock:
        islem_aciliyor.pop(sym, None)  # v16.60: açılış tamamen bitti, manage() artık normal takip edebilir
...
def tp_limit_emirlerini_koy(sym, direction, tp_liste, orijinal_qty):
    ...
    v16.60 DÜZELTME: Eskiden her TP'nin miktarı AYRI AYRI yuvarlanıyordu
...
def manage():
    while True:
        try:
            ...
            # v16.60: TERS SENKRONİZASYON — kullanıcı gözlemi: bazen trade_state'te
            ...
            # Bu döngü artık HER TURDA, trade_state'teki her sembolün borsada
            # GERÇEKTEN hâlâ açık olup olmadığını kontrol ediyor; değilse otomatik
            # temizliyor — kullanıcının manuel /kapat çalıştırmasına gerek kalmıyor.
            acik_semboller_borsada = {p["symbol"] for p in positions if safe(p.get("contracts")) > 0}
            with state_lock:
                izlenen_semboller = list(trade_state.keys())
            for sym in izlenen_semboller:
                if islem_aciliyor_mu(sym):
                    continue  # şu anda açılıyor, henüz borsaya yansımamış olabilir — atla
                if sym not in acik_semboller_borsada:
                    with state_lock:
                        eski_hard_stop = trade_state.get(sym, {}).get("hard_stop_id")
                        trade_state.pop(sym, None)
                    hard_stop_iptal_et(sym, eski_hard_stop)
                    durumu_diske_yaz()
                    tg(f"🧹 {sym} — borsada artık açık değildi (muhtemelen daha önce TP/SL ile kapanmış), "
                       f"eski kayıt otomatik temizlendi.")
            ...
            for p in positions:
                ...
                if not durum and islem_aciliyor_mu(sym):
                    # v16.60: Bu sembol TAM ŞU ANDA asil_islemi_ac() tarafından
...
SADECE_MANUEL = os.getenv("SADECE_MANUEL", "false").lower() == "true"  # v16.60: kullanıcı
...
if __name__ == "__main__":
    print("TELEGRAM SİNYAL KOPYALAMA BOTU (v16.60) BAŞLIYOR...")
    ...
    tg(
        ...
        "🔖 VERSİYON: v16.60 (SADECE MANUEL + TEYITLI + 4 TP + TP1 TABAN + 1H-VOLATILITE SL + ACIK-POZ DUZELTME + KURTARMA-TP + 4 sabit TP - VUR KAÇ %30/25/25/20 tam kapanış + hizli ac/kapat + teyit bekleme + "
