...
🔖 VERSİYON: v16.59 (SADECE MANUEL + TEYITLI + 4 TP + TP1 TABAN + 1H-VOLATILITE SL + ACIK-POZ DUZELTME + KURTARMA-TP + 4 sabit TP - VUR KAÇ %30/25/25/20 tam kapanış + hizli ac/kapat + teyit bekleme + kademeli SL yukseltme + 4-bilesenli trend teyidi (1h mum yonu dahil) + scalp oz tarama[VARSAYILAN KAPALI] + coklu kanal + manuel komutlar tekrar anında acilir (teyitsiz) + ANI HAREKET tespiti (Gir/Pas butonu))
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
# v16.59: AÇILIŞ-KURTARMA YARIŞ DURUMU DÜZELTMESİ.
...
# atlar.
islem_aciliyor = {}  # {symbol: baslama_zamani} — v16.59
...
PIN_MIN_ARALIK_SN = 10  # v16.59: art arda gelen SL/TP/hard-stop güncellemeleri
...
ANI_HAREKET_WATCHLIST_BOYUTU = int(os.getenv("ANI_HAREKET_WATCHLIST_BOYUTU", "30"))  # v16.59: 60->30, tur süresi kısalsın
...
ANI_HAREKET_TARAMA_ARALIK_SN = int(os.getenv("ANI_HAREKET_TARAMA_ARALIK_SN", "20"))  # v16.59: 45->20, daha hızlı yakalasın
...
def ani_hareket_1_2saat_tespit_et(sym):
    """
    v16.59 — kullanıcı geri bildirimi: "son 1 saatte 2 saatte yükselen/düşeni
    ...
    try:
        ...
        # v16.59: eşik %0.3'ten %0.6'ya çıkarıldı — kullanıcı gözlemi: %0.3
...
def ani_hareket_tarama_loop():
    ...
    # v16.59: RWA (tokenize edilmiş hisse/ETF, örn. MUUUSDT = Direxion MU Bull
        ...
        try:
            ...
            for sym, t in tickers.items():
                ...
                if sym in rwa_semboller:
                    continue  # v16.59: tokenize hisse/ETF — kripto gibi 7/24 işlem görmüyor, atla
                vol = safe(t.get("quoteVolume"))
                if vol < 3_000_000:  # v16.59: likidite tabanı düşürüldü (2.5M-8M arası
                    continue          # gerçek yükselenler de kaçmasın diye)
                # v16.59: EN ÖNEMLİ DEĞİŞİKLİK — kullanıcı geri bildirimi: "şu an
            ...
            for sym in watchlist:
                ...
                # v16.59: Kullanıcı kararı — hız yerine GÜVENİLİRLİK tercih edildi.
                ...
                # v16.59: kullanıcı kararı DEĞİŞTİ — "otomatik işlem açmayı
                ...
                #   1) SADECE LONG (v16.59'ten kalan karar hâlâ geçerli)
...
def efektif_kaldirac_al(sym):
    """
    v16.59: Her coin'in borsadaki MAKSİMUM izin verilen kaldıracı farklı
...
def pozisyon_boyutu_hesapla(entry, sl, lev=None):
    ...
    v16.59: `lev` parametresi eklendi — verilmezse global LEV kullanılır,
...
def gercek_bakiye_yeterli_mi(gereken_marj):
    ...
    v16.59 DÜZELTME: STRK/USDT gibi örneklerde, bu kontrol GEÇMESİNE rağmen
    ...
    gereken_marj_payli = gereken_marj * 1.08  # v16.59: %8 güvenlik payı
...
# ── TP DİLİM AĞIRLIKLARI (v16.59 — 4 SABİT TP + KALAN %15 TRAILING) ──
...
MIN_TP1_R_ORANI = 0.65  # v16.59: 0.40'tan 0.65'e yükseltildi. Gerçek 10 işlemlik
...
def sinyal_guc_skoru(rsi_1h, mum_4h_sayisi, mum_1h_sayisi, direction):
    """
    v16.59 — kullanıcı talebi: "sinyale yüzde koysan, çok güçlüyse bilelim".
...
def trend_teyidi_yeterli_mi(sym, direction):
    ...
    try:
        ...
        if direction == "long":
            ...
            rsi_yeterli = 40 < rsi_1h < 80  # v16.59: ÜST SINIR eklendi — RSI 92.9
            ...
            rsi_yeterli = 20 < rsi_1h < 60  # v16.59: ALT SINIR eklendi — aynı mantık
...
def sinyali_isle(sinyal):
    ...
    # v16.59: kullanıcı kararı — manuel komutlar, kanal sinyalleri ve ani
...
def asil_islemi_ac(sinyal, gozlem_str=""):
    ...
    # v16.59: SEMBOL ADI UYUŞMAZLIĞI düzeltmesi — kullanıcı gözlemi: bazı
    ...
    # v16.59: RWA (tokenize hisse/ETF) GÜVENLİK KONTROLÜ — hangi yoldan
    ...
    # v16.59: coin'in GERÇEK maksimum kaldıracı önce belirlenip, tüm
    ...
    with islem_aciliyor_lock:
        islem_aciliyor[sym] = time.time()  # v16.59: manage()'in kurtarma bloğu bu sembolü açılış bitene kadar atlasın

    # v16.59: "amount exceeds balance" (40762) hatasına karşı otomatik
    ...
    with islem_aciliyor_lock:
        islem_aciliyor.pop(sym, None)  # v16.59: açılış tamamen bitti, manage() artık normal takip edebilir
...
def tp_limit_emirlerini_koy(sym, direction, tp_liste, orijinal_qty):
    ...
    v16.59 DÜZELTME: Eskiden her TP'nin miktarı AYRI AYRI yuvarlanıyordu
...
def manage():
    while True:
        try:
            ...
            # v16.59: TERS SENKRONİZASYON — kullanıcı gözlemi: bazen trade_state'te
            ...
            for p in positions:
                ...
                if not durum and islem_aciliyor_mu(sym):
                    # v16.59: Bu sembol TAM ŞU ANDA asil_islemi_ac() tarafından
...
SADECE_MANUEL = os.getenv("SADECE_MANUEL", "false").lower() == "true"  # v16.59: kullanıcı
...
if __name__ == "__main__":
    print("TELEGRAM SİNYAL KOPYALAMA BOTU (v16.59) BAŞLIYOR...")
    ...
    tg(
        ...
        "🔖 VERSİYON: v16.59 (SADECE MANUEL + TEYITLI + 4 TP + TP1 TABAN + 1H-VOLATILITE SL + ACIK-POZ DUZELTME + KURTARMA-TP + 4 sabit TP - VUR KAÇ %30/25/25/20 tam kapanış + hizli ac/kapat + teyit bekleme + "
