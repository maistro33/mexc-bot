#!/usr/bin/env python3
"""
SADIK PAPER TRADING BOT v11
XGBoost Ana Karar Verici + Online Learning
Kurallar sadece koruma katmanı
"""
...
MAX_OPEN      = 10
SCAN_INTERVAL = 40
AI_MIN_SCORE  = 55   # Daha yüksek eşik — model ana karar verici
...
def _build_row_from_simple(rec):
    return {
        "rsi":          float(rec.get("rsi") or 50),
        "volume_ratio": float(rec.get("vol_ratio") or rec.get("volume_ratio") or 1),
        "momentum":     0,
        "move_1":       0,
        "move_3":       float(rec.get("move_3") or 0),
        "volatility":   0,
...
def train_ai_model():
    ...
    try:
        ...
        with _model_lock:
            ...
            _model = model; _scaler = scaler; _model_trained = True

        log.info(f"[AI] Model eğitildi! %{acc*100:.1f} | {len(df)} işlem")
...
def online_update(ind, btc_trend, signal, pnl):
    global _model, _scaler, _model_trained
    if not _model_trained or _model is None or _scaler is None: return
    try:
        row = {
            "rsi":          ind.get("rsi", 50),
...
def xgboost_decision(ind, btc_trend, signal):
    """
    ANA KARAR VERİCİ — XGBoost LONG/SHORT/PAS döndürür
    Kurallar değil, model karar verir
    """
    global _model, _scaler
    if not _model_trained or _model is None or _scaler is None:
        return signal, 60  # Model yoksa kurala dön
    ...
    try:
        ...
        row_long = {
            ...
            "btc_up":       1 if btc_trend == "UP" else 0,
            "btc_down":     1 if btc_trend == "DOWN" else 0,
            "signal_long":  1,  # LONG senaryosu
        }
        # SHORT tahmini
        row_short = dict(row_long)
        row_short["signal_long"] = 0  # SHORT senaryosu

        with _model_lock:
            X_long  = pd.DataFrame([row_long])[FEATURES]
            X_short = pd.DataFrame([row_short])[FEATURES]
            prob_long  = _model.predict_proba(_scaler.transform(X_long))[0][1]
            prob_short = _model.predict_proba(_scaler.transform(X_short))[0][1]

        score_long  = int(prob_long  * 100)
        score_short = int(prob_short * 100)

        log.info(f"[AI] LONG:{score_long} SHORT:{score_short}")

        # Model en iyi kararı seçiyor
        if score_long >= score_short and score_long >= AI_MIN_SCORE:
            return "LONG", score_long
        elif score_short > score_long and score_short >= AI_MIN_SCORE:
            return "SHORT", score_short
        else:
            return None, max(score_long, score_short)  # İkisi de yetersiz

    except Exception as e:
        log.warning(f"[AI] Karar hatası: {e}")
        return signal, 60
...
# ─── KORUMA FİLTRELERİ (kurallar sadece burada) ───
def safety_check(ind, btc_trend):
    """
    XGBoost ana karar verici, bu fonksiyon sadece
    tehlikeli durumları engeller
    """
    # Fake pump/dump engelle
    if ind.get("fake_up") or ind.get("fake_down"):
        return False, "Fake pump/dump"

    # Çok düşük hacim engelle
    if ind.get("vol_ratio", 0) < 0.5:
        return False, "Hacim çok düşük"

    # Aşırı RSI engelle
    rsi = ind.get("rsi", 50)
    if rsi < 20 or rsi > 85:
        return False, f"Aşırı RSI: {rsi:.0f}"
...
Sadece GİR veya PAS yaz, 1 cümle neden."""
        ...
        return True, f"GPT {r.status_code} — otomatik gir"
    except Exception as e:
        log.warning(f"[GPT] {e}")
    ...
    while True:
        try:
            ...
            for coin in active:
                ...
                # ─── KORUMA FİLTRESİ ───
                safe, reason = safety_check(ind, btc_trend)
                if not safe:
                    log.debug(f"[SAFETY] {symbol.split('/')[0]} {reason}")
                    continue

                # ─── XGBOOST ANA KARAR ───
                signal, score = xgboost_decision(ind, btc_trend, "LONG")

                if signal is None:
                    log.info(f"[SKIP] {symbol.split('/')[0]} XGBoost pas: {score}")
                    continue
                ...
                sym=symbol.split("/")[0]
                log.info(f"[GPT] {sym} {signal} → {'GİR' if gir else 'PAS'} | {yorum}")

                if not gir: continue

                log.info(f"[SİNYAL] {sym} {signal} RSI={ind['rsi']:.0f} score={score}")
...
def health_server():
    ...
    class H(BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            status=f"OK | pos:{len(positions)} | ai:{_model_trained}"
...
def cmd_stats(msg):
    ...
    try:
        ...
        bot.send_message(msg.chat.id,
            ...
            f"🤖 XGBoost Ana Karar Verici + Online Learning"
        )
    except Exception as e:
        log.error(f"[STATS] {e}")
...
if __name__=="__main__":
    print("📋 SADIK PAPER TRADING BOT v11 BAŞLIYOR...")
    load_ai_model()
    ...
    tg(
        "📋 SADIK PAPER TRADING BOT v11\n\n"
        "🤖 XGBoost ANA KARAR VERİCİ!\n\n"
        "✅ Model LONG/SHORT/PAS karar veriyor\n"
        "✅ Kurallar sadece koruma katmanı\n"
        "✅ Online Learning — her işlemde güncelleme\n"
        "✅ Her 6 saatte tam yeniden eğitim\n"
        "✅ GPT opsiyonel onay\n\n"
        f"AI_MIN_SCORE: {AI_MIN_SCORE} | Max:{MAX_OPEN} pozisyon\n\n"
