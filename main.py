...
ESKİ (v5.24): "son 15dk'da %1.5+ yükseldiyse hemen gir" (trend
kovalama, SADECE LONG). 287 gerçek işlemde simetrik sonuç verdi
(+%3.34 kazanç / -%3.44 kayıp) - gerçek edge yoktu.

YENİ (v6.0): "düşükten al, yüksekten sat" - SWING DİP/TEPE + DÖNÜŞ
ONAYI, HEM LONG HEM SHORT, hiçbir indikatör yok (RSI/ADX/hacim YOK):
  - LONG: son LOOKBACK mumun (5m) EN DÜŞÜĞÜ yakın zamanda (son 3 mum
    içinde) yapıldı VE şu anki mum yukarı kapandı
  - SHORT: aynı mantık tersten (tepe + aşağı kapanış)

SL artık ATR değil, SWING NOKTASI bazlı - noktanın biraz ötesine
(SL_BUFFER_PCT) konur, taban (MIN_SL_PCT) ve TARGET_MAX_LOSS_USDT
hedefinden türeyen tavan arası sınırlanır. Kullanıcı talebi: "SL
geniş olsun, likide olmadan önce kapansın, hedef ~$0.90 kayıp."

Backtest (50 coin, 10 gün, gerçek Bitget verisi, 11.08.2026):
LONG tek başına 701 işlemde %46.4 kazanma, net +$6.47 (hafif
pozitif). SHORT 809 işlemde %36.2 kazanma, net -$72.84 (belirgin
negatif) - kullanıcı onayıyla ikisi de açık, canlı veriyle
ayrıca izlenecek.

Çıkış: SL (swing bazlı, geniş) + İZ SÜREN TP (v5.24 ile aynı mekanizma,
1.0R'de aktifleşir, 0.5R geri çekilirse kapanır - bkz. IZ_SURME_R_ORANI).

Coin tarama mantığı, panel sistemi, coin engelleme, AJAN 0/1 mimarisi,
tüm Telegram komutları v5.24 ile BİREBİR AYNI - değişmedi.
════════════════════════════════════════════════════════
"""
...
ATR_CARPANI_SL = 2.0  # artık swing_nokta varsa kullanılmıyor, sadece yedek/güvenlik yolu

# KULLANICI KARARI (11.08.2026): swing dip/tepe sinyaline geçişle birlikte
# SL artık ATR yerine SWING NOKTASI bazlı hesaplanıyor (bkz.
# _islem_acici_pozisyon_ac_ic). LOOKBACK = dip/tepe kaç mumluk pencerede
# aranıyor (5m mum). SL_BUFFER_PCT = swing noktasının ne kadar ötesine SL
# konur (ekstra güvenlik payı).
LOOKBACK = int(os.getenv("LOOKBACK", "20"))
SL_BUFFER_PCT = float(os.getenv("SL_BUFFER_PCT", "0.015"))

# KULLANICI KARARI (11.08.2026): "SL geniş olsun, likide olmadan (Cross'ta
# ~%10 hareket) önce kapansın, hedef ~$0.90 kayıp" - MAX_SL_PCT artık bu
# hedeften türetiliyor (bkz. efektif_max_sl_pct hesabı), burada sadece son
...
def piyasa_izleyici_swing_sinyal(sym, btc_bullish):
    """KULLANICI KARARI (11.08.2026): trend kovalama mantığı 287 gerçek
    işlemde simetrik sonuç verdi (+%3.34 kazanç / -%3.44 kayıp, gerçek
    edge yoktu - bkz. panel_analiz geçmişi). Yerine SWING DİP/TEPE +
    DÖNÜŞ ONAYI mantığı getirildi - "düşükten al, yüksekten sat",
    HİÇBİR indikatör yok (RSI/ADX/hacim YOK), sadece fiyat aksiyonu:

      LONG: son LOOKBACK mumun EN DÜŞÜĞÜ yakın zamanda (son 3 mum
      içinde) yapıldı VE şu anki mum yukarı kapandı (dip + dönüş onayı)

      SHORT: son LOOKBACK mumun EN YÜKSEĞİ yakın zamanda yapıldı VE şu
      anki mum aşağı kapandı (tepe + dönüş onayı)

    Backtest (50 coin, 10 gün, gerçek Bitget verisi, 11.08.2026):
    LONG tek başına 701 işlemde %46.4 kazanma, net +$6.47 (hafif
    pozitif). SHORT 809 işlemde %36.2 kazanma, net -$72.84 (belirgin
    negatif) - kullanıcı onayıyla ikisi de açık tutuluyor, canlı veri
    ile ayrıca izlenecek."""
    df5 = get_df(sym, "5m", GOSTERGE_MUM_5M)
    if df5 is None or len(df5) < LOOKBACK + 2:
        return None

    df5["atr"] = atr(df5, 14)
    pencere = df5.iloc[-(LOOKBACK + 1):-1]
    son_mum = df5.iloc[-1]

    if pd.isna(son_mum["atr"]) or son_mum["atr"] <= 0:
        return None
    ...
    if dip_yakin and yukari_kapandi and son_mum["close"] > swing_low:
        return {"symbol": sym, "entry": float(son_mum["close"]), "atr": float(son_mum["atr"]),
                "skor": 0.0, "tur": "swing_dip", "swing_nokta": float(swing_low),
                "tetik_ts": int(son_mum["ts"])}

    if tepe_yakin and asagi_kapandi and son_mum["close"] < swing_high:
        return {"symbol": sym, "entry": float(son_mum["close"]), "atr": float(son_mum["atr"]),
                "skor": 0.0, "tur": "swing_tepe", "swing_nokta": float(swing_high),
                "tetik_ts": int(son_mum["ts"])}
...
def sinyal_yonu(tur):
    return "short" if tur in ("dusus_devam", "swing_tepe") else "long"
...
def islem_acici_pozisyon_ac(sinyal, kaynak="tarama"):
    ...
    entry = sinyal["entry"]
    atr_val = sinyal["atr"]
    swing_nokta = sinyal.get("swing_nokta")
    ...
    try:
        _islem_acici_pozisyon_ac_ic(sym, entry, atr_val, tur, yon, kaynak, swing_nokta)
    finally:
        with state_lock:
            acilis_rezervasyonlari.pop(sym, None)


def _islem_acici_pozisyon_ac_ic(sym, entry, atr_val, tur, yon, kaynak="tarama", swing_nokta=None):
    ...
    # KULLANICI KARARI (11.08.2026): SL artık ATR bazlı değil, SWING NOKTASI
    # bazlı hesaplanıyor - "geniş SL, likide olmadan (Cross'ta ~%10) önce
    # kapansın, hedef ~$0.90 kayıp" talebiyle. Swing noktasının biraz
    # ötesine (SL_BUFFER_PCT) konur, taban (MIN_SL_PCT) ve TARGET_MAX_LOSS_USDT
    # hesabından türeyen tavan (aşağıda hesaplanan efektif_max_sl_pct) arası
    # sınırlanır. swing_nokta yoksa (beklenmez ama güvenlik için) eski
    # ATR bazlı hesaba düşülür.
    LEV_ON = sembol_max_kaldirac(sym, LEV)
    notional_on = (SABIT_MARJIN_USDT * LEV_ON) if SABIT_MARJIN_USDT > 0 else (bakiye * RISK_PCT_BAKIYE * LEV_ON)
    efektif_max_sl_pct = TARGET_MAX_LOSS_USDT / notional_on if notional_on > 0 else MAX_SL_PCT
    efektif_max_sl_pct = min(efektif_max_sl_pct, MAX_SL_PCT) if MAX_SL_PCT > 0 else efektif_max_sl_pct

    if yon == "long":
        if swing_nokta is not None:
            sl = swing_nokta * (1 - SL_BUFFER_PCT)
        else:
            sl = entry - ATR_CARPANI_SL * atr_val
        if (entry - sl) / entry > efektif_max_sl_pct:
            sl = entry * (1 - efektif_max_sl_pct)
        ...
        if swing_nokta is not None:
            ...
            sl = entry + ATR_CARPANI_SL * atr_val
        if (sl - entry) / entry > efektif_max_sl_pct:
            sl = entry * (1 + efektif_max_sl_pct)
    ...
    tur_etiket = "dip dönüşü (LONG)" if tur == "swing_dip" else ("tepe dönüşü (SHORT)" if tur == "swing_tepe" else ("ani patlama" if tur == "spike" else ("sürdürülebilir tırmanış" if tur == "sustained" else ("düşüş devamı" if tur == "dusus_devam" else tur))))
    ...
    tg(f"{yon_emoji} SWING POZİSYON: {sym} {yon_etiket} [{tur_etiket}]\n"
       f"Giriş≈{entry:.6f} | SL:{sl:.6f} (swing bazlı, hedef kayıp≈${TARGET_MAX_LOSS_USDT:.2f})\n"
       f"{tp_ozet}\n"
       f"{risk_ozet}")
...
if bot:
    ...
    def panel_ayarlar_metni():
        return ("⚙️ SWING BOT AYARLARI\n\n"
                f"Sürüm: v6.0 (v5.24 scalp_bot mimarisi korunuyor - panel, komutlar, "
                f"coin engelleme, iz süren TP AYNI; SADECE sinyal SWING DİP/TEPE + DÖNÜŞ "
                f"ONAYINA değişti - artık HEM LONG HEM SHORT, hiçbir indikatör yok; "
                f"SL artık ATR değil SWING NOKTASI bazlı, hedef kayıp≈$0.90; "
                f"11.08.2026) — "
                f"RSI/ADX/hacim-spike/üst-fitil filtreleri YOK. "
                f"Sadece fiyat aksiyonu ile hemen giriş, SL + geniş iz süren TP ile çıkış.\n"
                ...
                f"GİRİŞ: son {LOOKBACK} mumda (5m) dip/tepe yapılıp dönüş onayı gelirse "
                f"HEMEN girilir (LONG: dip+yukarı kapanış, SHORT: tepe+aşağı kapanış, "
                f"indikatör yok, sadece fiyat aksiyonu)\n"
                f"SL: swing noktası bazlı (LOOKBACK={LOOKBACK} mum), hedef kayıp≈${TARGET_MAX_LOSS_USDT:.2f}, taban %{MIN_SL_PCT*100:.0f}\n"
    ...
    def panel_gecmis_metni():
        ...
        for t in list(reversed(gecmis))[:15]:
            tur = t.get("tur", "?")
            tur_kisa = "dip dönüşü" if tur == "swing_dip" else ("tepe dönüşü" if tur == "swing_tepe" else ("patlama" if tur == "spike" else ("sürdürülebilir" if tur == "sustained" else ("düşüş devamı" if tur == "dusus_devam" else tur))))
            sebep = t.get("not", "")
            emoji = "🟢" if t["pnl"] >= 0 else "🔴"
    ...
    def panel_analiz_metni():
        ...
        for tur in turler:
            ...
            tur_ad = {"spike": "Ani patlama (eski)", "sustained": "Sürdürülebilir tırmanış (eski)",
                      "dusus_devam": "Düşüş devamı (eski)", "basit_trend": "Sade trend (eski)",
                      "swing_dip": "Dip dönüşü (LONG)", "swing_tepe": "Tepe dönüşü (SHORT)"}.get(tur, tur)
