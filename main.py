...
def cmd_genel(msg):
    ...
    try:
            ...
            if positions:
                ...
                for sym, pos in list(positions.items()):
                    ...
                    if t:
                        ...
                        pnl = (price-entry)/entry*pos_size if signal=="LONG" else (entry-price)/entry*pos_size
                        sure = int((time.time()-pos["open_time"])/60)
                        pos_bilgi_list.append(f"{sym.split('/')[0]} {signal} PnL:{pnl:+.2f} USDT {sure}dk")
                pos_bilgi = "\n".join(pos_bilgi_list)
            else:
        ...
        system_msg = (
            SYSTEM_PROMPT + "\n\n"
            "Sen kullanicinin trading asistanisin. Dogal Turkce konusuyorsun.\n"
            "Kullanici coin analizi isterse, analiz et ve uygunsa isle.\n"
            "JSON formatinda islem komutu verebilirsin:\n"
            'Islem ac: {"islem_ac": true, "symbol": "AVAX/USDT:USDT", "karar": "LONG", "tp_pct": 2.0, "sl_pct": 1.0, "guven": 80, "neden": "..."}\n'
            'Pozisyon kapat: {"kapat": "AVAX"}\n'
            'Hepsini kapat: {"hepsini_kapat": true}\n'
            "Sadece soru soruyorsa normal cevap ver. Kisa ve net ol."
        )
        ...
        if json_match:
            try:
                ...
                if karar.get("islem_ac") and karar.get("symbol"):
                    ...
                    with pos_lock: syms = list(positions.keys())
                    for s in syms:
                        close_paper(s, "Kullanici hepsini kapat")

            except Exception as je:
                log.warning(f"[GENEL JSON] {je}")

        # Metni temizle
        temiz = re.sub(r'\{[^{}]+\}', '', yanit).strip()
        bot.send_message(msg.chat.id, f"🤖 {temiz if temiz else yanit[:400]}")

    except Exception as e:
        log.error(f"[GENEL] {e}")
        bot.send_message(msg.chat.id, f"Hata olustu: {type(e).__name__}")
