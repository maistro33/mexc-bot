...
def open_test_trade():
    """Bot başlar başlamaz bağlantıyı doğrulamak için 1 işlem açar."""
    try:
        symbol = 'DOGE/USDT:USDT'
        ticker = ex.fetch_ticker(symbol)
        p = float(ticker['last'])
        amt = (10.0 * CONFIG['leverage']) / p # 10 USDT bakiye ile test
        
        ex.set_leverage(CONFIG['leverage'], symbol)
        # Senin hatasız çalışan emir yapın
...
# --- [5. ANA DÖNGÜ] ---
def main_loop():
    send_msg("🚀 **RADAR BAŞLATILDI**\n300+ Coin taranıyor...")
    
    # --- TESTİ BURADA TETİKLİYORUZ ---
    open_test_trade()
