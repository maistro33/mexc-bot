...
def ai_decision(sym,score,ob):
    try:
        ...
        if d=="LONG" and trend!="UP": return "SKIP"
        if d=="SHORT" and trend!="DOWN": return "SKIP"
...
def anti_dump(sym,pnl):
    ...
    if time.time()-st["open_time"]<60:return False
    if pnl>-0.5:return False
    ...
    while True:
        try:
            for p in exchange.fetch_positions():
                ...
                if not st["tp1"] and r>=TP_RR:
                    tp_qty=float(exchange.amount_to_precision(sym,qty*0.5))
                ...
                for lvl in STEP_LEVELS:
                    if r>=lvl and st["step"]<lvl:
                        st["step"]=lvl
                        st["sl"]=entry+(lvl-1)*st["risk"] if direction=="long" else entry-(lvl-1)*st["risk"]
...
bot.send_message(CHAT_ID,"🔥 FINAL STABLE AI")
