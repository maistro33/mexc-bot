...
# v16.17: BTC/ETH/SOL gibi buyuk, "hantal" coinler kullanicinin ISTEMEDIGI
...
# v16.17: hacim bandinin YANINDA, son 24 saatte SERT hareket etmis
# coinler de (kullanicinin ekran goruntusundeki +%47, -%17 gibi
# ornekler) dahil ediliyor - bunlar hacim bandinin disinda kalsa bile
# "hareketli/scalp" tanimina giriyor. Esik degeri %10 (0.10).
OZ_TARAMA_MIN_DEGISIM_PCT = float(os.getenv("OZ_TARAMA_MIN_DEGISIM_PCT", "0.10"))
OZ_TARAMA_WATCHLIST_BOYUTU = int(os.getenv("OZ_TARAMA_WATCHLIST_BOYUTU", "25"))

# v16.17: BUYUK/KOKLU COIN KARA LiSTESi - hacim bandi ya da %degisim
# esigi tek basina yeterli degil (LTC ornegi: hacmi gecici dustugunde
# banda giriyordu; BTC de bazen %10+ hareket edip degisim esigini
# gecebilir). Bu liste, hacim/degisim ne olursa olsun bu coinlerin ASLA
# oz tarama tarafindan acilmamasini garanti eder.
OZ_TARAMA_KARA_LISTE = set(
    x.strip().upper() for x in os.getenv(
        "OZ_TARAMA_KARA_LISTE",
        "BTC,ETH,SOL,BNB,XRP,LTC,ADA,DOT,DOGE,AVAX,LINK,MATIC,POL,TRX,BCH"
    ).split(",") if x.strip()
)
...
def oz_tarama_watchlist_getir():
    """
    v16.17: Iki ayri kriterle aday toplar, birlestirir:
      A) Hacim bandi ($1M-$5M gibi) - "sakin/orta likiditeli scalp" coinler
      B) |24s degisim| >= esik (%10) - "sert hareket etmis" coinler,
         hacim bandinin disinda olsalar bile
    Ardindan: kara listedeki (BTC/ETH/SOL/LTC vb.) coinler ve RWA urunleri
    HER ZAMAN elenir - hangi kritere uysalar da.
    """
    try:
        r = requests.get("https://api.bitget.com/api/v2/mix/market/tickers",
                          params={"productType": "USDT-FUTURES"}, timeout=15)
        ham_tickers = r.json().get("data", [])
    except Exception as e:
        log.warning(f"[OZ_TARAMA] Ticker listesi alinamadi: {e}")
        return []

    hacim_bandi_adaylari = []
    degisim_adaylari = []  # (sym, |degisim|, imzali_degisim)
    for t in ham_tickers:
        bitget_sym = t.get("symbol", "")
        if not bitget_sym.endswith("USDT"):
            continue
        coin = bitget_sym[:-4]
        sym = f"{coin}/USDT:USDT"

        # v16.17: kara listedeki coinler hicbir kritere bakilmadan en basta elenir
        if coin.upper() in OZ_TARAMA_KARA_LISTE:
            continue

        hacim = safe(t.get("usdtVolume"))
        degisim = safe(t.get("change24h"))  # orn. 0.0175 = %1.75, -0.17 = %-17

        if OZ_TARAMA_MIN_HACIM_USDT <= hacim <= OZ_TARAMA_MAX_HACIM_USDT:
            hacim_bandi_adaylari.append((sym, hacim))
        if abs(degisim) >= OZ_TARAMA_MIN_DEGISIM_PCT:
            degisim_adaylari.append((sym, abs(degisim), degisim))

    # v16.17: hacim bandi adaylari KUCUKTEN BUYUGE - bandin ust sinirina
    # yakin (koklu/taninmis) coinler yerine gercekten kucuk olanlar oncelikli
    hacim_bandi_adaylari.sort(key=lambda x: x[1], reverse=False)
    # degisim adaylari BUYUKTEN KUCUGE (mutlak deger) - en sert hareket
    # edenler oncelikli
    degisim_adaylari.sort(key=lambda x: x[1], reverse=True)

    # v16.17: iki listeyi ic ice orerek birlestir - her adayin hangi
    # kriterden geldigini ve (varsa) IMZALI degisim yuzdesini tasiyoruz;
    # boylece oz_tarama_loop, "degisim" kaynakli adaylar icin YONU
    # DOGRUDAN HAREKETIN TERSINE (dipten LONG, tepeden SHORT) belirleyebilir.
    birlesik = []
    gorulen = set()
    i = j = 0
    while len(birlesik) < OZ_TARAMA_WATCHLIST_BOYUTU * 2 and (i < len(hacim_bandi_adaylari) or j < len(degisim_adaylari)):
        if i < len(hacim_bandi_adaylari):
            sym = hacim_bandi_adaylari[i][0]
            if sym not in gorulen:
                birlesik.append({"symbol": sym, "kaynak": "hacim", "degisim_pct": None})
                gorulen.add(sym)
            i += 1
        if j < len(degisim_adaylari):
            sym, _, imzali_degisim = degisim_adaylari[j]
            if sym not in gorulen:
                birlesik.append({"symbol": sym, "kaynak": "degisim", "degisim_pct": imzali_degisim})
                gorulen.add(sym)
            j += 1

    # v16.14: RWA/tokenize hisse-ETF urunlerini watchlist'ten cikar
    temiz_liste = [aday for aday in birlesik if not rwa_mi(aday["symbol"])]
    ...
    while True:
        try:
            ...
            for aday in watchlist:
                ...
                if aday["kaynak"] == "degisim":
                    # v16.17: KONTRARYAN MANTIK - kullanici talebiyle: sert
                    # DUSMUS (negatif degisim) coinler LONG adayi (dipten
                    # alim), sert YUKSELMIS (pozitif degisim) coinler SHORT
                    # adayi (tepeden satim) sayilir. Trend-takip MA20/RSI
                    # filtresi buraya BILEREK uygulanmiyor - ikisi zaten
                    # birbirine ters mantik (trend-takip "dipte LONG acma"
                    # derdi, ama kullanicinin istedigi tam olarak bu).
                    degisim_pct = aday["degisim_pct"]
                    yon = "long" if degisim_pct < 0 else "short"
                    kaynak_aciklama = f"sert %{degisim_pct*100:+.1f} hareket - kontraryan"
                else:
                    yon = oz_tarama_aday_degerlendir(sym)
                    kaynak_aciklama = "trend teyidi (MA20+RSI)"
                ...
                sinyal = {"symbol": sym, "direction": yon, "entry": None, "sl": None,
                          ...
                          "atla_teyit": (aday["kaynak"] == "degisim")}
                tg(f"[OZ TARAMA] {sym} {yon.upper()} adayi bulundu ({kaynak_aciklama}, taze) - isleniyor...")
                sinyali_isle(sinyal)
                with state_lock:
...
def sinyali_isle(sinyal):
    ...
    # v16.17: KONTRARYAN sinyaller (oz_tarama'nin "degisim" kaynagi -
    # dipten LONG / tepeden SHORT) BILEREK trend teyidinin TERSI yonde
    # calisir - trend teyidi burada anlamsizdir (zaten "trend zayif/ters"
    # oldugu icin kontraryan firsat sayiliyor). Bu yuzden "atla_teyit"
    # bayragi tasiyan sinyaller bu kontrolden GECIRILMEDEN dogrudan acilir
    # (likidite kontrolu hala gecerli, sadece trend teyidi atlaniyor).
    if sinyal.get("atla_teyit"):
        asil_islemi_ac(sinyal, gozlem_str + " | KONTRARYAN (trend teyidi bilerek atlandi)")
        return
...
if __name__ == "__main__":
    print("TELEGRAM SINYAL KOPYALAMA BOTU (v16.17) BASLIYOR...")
    ...
    tg(
        ...
        "VERSIYON: v16.17 (hareketli/scalp coin hacim bandi + RWA filtresi + risk-bazli pozisyon + likidite kontrolu)\n\n"
