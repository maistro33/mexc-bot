...
ex = ccxt.bitget({
    ...
    'options': {
        'defaultType': 'swap',
        'hedged': True,           # Hedge Mode zorunlu
    },
...
def place_tpsl(symbol, plan_type, trigger_price, hold_side, qty):
    ...
    try:
        ...
        bot.send_message(MY_CHAT_ID, f"✅ {plan_type.upper().replace('_', ' ')} eklendi → {symbol} @ {trigger_price}")
        return order
    ...
    while symbol in active_trades:
        try:
            ...
            if not pos or float(pos[0].get('contracts', 0)) == 0:
                ...
                bot.send_message(MY_CHAT_ID, f"🏁 {symbol} kapandı.")
                break
        except: break
